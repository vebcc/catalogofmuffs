<html lang="pl">
    <head>
        ...
        if($local==false){
            if(!isset($_SESSION["error"])){ // czy sesja zawiera jakis error
                if(!isset($_SESSION["login"]) && !isset($_SESSION['token']) && !isset($_SESSION['token2'])){ // czy zmienne sesji nie sa ustawione
                        ...
                        if($ifban==0){ // sprawdzanie permbana
                            ...
                            if($db_row["banned"]==0){ // ban tymczasowy
                                ...
                                if($pwd == $db_row["password"]){ // czy hasla sie zgadzaja
                                    ...
                                    $db_query = mysqli_query($con,"INSERT INTO `comfs`.`log` (`id`, `error`, `ip`, `date`, `browser`) VALUES (NULL, 'globallogin', '$actual_ip', CURRENT_TIMESTAMP, '".$_SERVER['HTTP_USER_AGENT']."');");
                                    ...
                                    $db_query = mysqli_query($con,"INSERT INTO `comfs`.`log` (`id`, `error`, `ip`, `date`, `browser`) VALUES (NULL, 'faillogin', '$actual_ip', CURRENT_TIMESTAMP, '".$_SERVER['HTTP_USER_AGENT']."');");
                                    ...
                                    if($db_row["ilosc"]>=3){ // sprawdzanie czy klient o danym ip dostal juz 3 bany w ciagu 10 minut
                                        ...
                                        $db_query = mysqli_query($con,"INSERT INTO `comfs`.`log` (`id`, `error`, `ip`, `date`, `browser`) VALUES (NULL, 'loginban', '$actual_ip', CURRENT_TIMESTAMP, '".$_SERVER['HTTP_USER_AGENT']."');");
                    ...
                    if($_SESSION['token']==md5($_SERVER['HTTP_USER_AGENT']) && $_SESSION['token2']==md5($_SERVER['REMOTE_ADDR'])){ // sprawdzanie czy dane z sesji zgadzaja sie z danymi klienta
                        ...
                        require("comfs.php"); // include panel admina
                    }else{
                ...
                $db_query = mysqli_query($con,"INSERT INTO `comfs`.`log` (`id`, `error`, `ip`, `date`, `browser`) VALUES (NULL, 'interference', '$actual_ip', CURRENT_TIMESTAMP, '".$_SERVER['HTTP_USER_AGENT']."');");
            ...
            $db_query = mysqli_query($con,"INSERT INTO `comfs`.`log` (`id`, `error`, `ip`, `date`, `browser`) VALUES (NULL, 'locallogin', '$actual_ip', CURRENT_TIMESTAMP, '".$_SERVER['HTTP_USER_AGENT']."');");
